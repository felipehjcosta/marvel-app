...
inline fun <reified T : View> Fragment.bindView(@IdRes idRes: Int): Lazy<T> {
    return lazy(LazyThreadSafetyMode.NONE) { view?.findViewById(idRes) as T }
}
...
inline fun <reified T : View> RecyclerView.ViewHolder.bindView(@IdRes idRes: Int): Lazy<T> {
    return lazy(LazyThreadSafetyMode.NONE) { itemView.findViewById(idRes) as T }
}

inline fun <reified T : View> View?.findBy(@IdRes res: Int): T = this?.findViewById(res) as T
inline fun <reified T : View> Activity?.findBy(@IdRes res: Int): T = this?.findViewById(res) as T
