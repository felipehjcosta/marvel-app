...
class LoadingOthersCharacterItemRecyclerViewAdapter(
        size: Int
) : RecyclerView.Adapter<LoadingOthersCharacterItemRecyclerViewAdapter.ViewHolder>() {

    private val items = arrayOfNulls<Any?>(size)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = from(parent.context)
            .inflate(R.layout.loading_others_characters_item, parent, false).run {
                ViewHolder(this)
            }
